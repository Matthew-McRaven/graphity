...
class sync_autocorrelation:
	...
	def run(self,):
		...
		for trajectory in trajectories:
			first_below_zero = [idx for idx, item in enumerate(trajectory) 
				if self.chi(idx, self.sweeps, trajectory)/self.chi(0, self.sweeps, trajectory)<0][0]
			...
			act = functools.reduce(lambda sum, t:sum+self.chi(t, first_below_zero, trajectory)/norm, 
	...
	def run(self,):
		...
		for trajectory in trajectories:
			...
			below_zero = [idx for idx, item in enumerate(trajectory) 
				if self.chi(idx, self.sweeps, trajectory)/self.chi(0, self.sweeps, trajectory)<0]
