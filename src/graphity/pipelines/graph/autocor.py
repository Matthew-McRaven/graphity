...
def chi(t, t_max, trajectory):
	...
	for t_prime in range(0, t_max - t):
		term_0 += to_spin(trajectory[t_prime]['state']).float().mean() *  to_spin(trajectory[t_prime+t]['state']).float().mean()
		term_1 += to_spin(trajectory[t_prime]['state']).float().mean()
		term_2 += to_spin(trajectory[t]['state']).float().mean()
...
class sync_autocorrelation:
	...
	def run(self,):
		tasks = [graphity.pipelines.graph.create_eq_task(idx, self.beta, self.eq_lattices[0].shape, self.H) for idx in range(self.count)]
