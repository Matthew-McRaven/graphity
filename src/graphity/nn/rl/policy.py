...
class BiCategoricalPolicy:
    ...
    def log_prob(self, actions):
        def get_depth(fn, depth = 0, count = 0):
            max_depth = depth 
            for child in [n[0] for n in fn.next_functions if n[0]]:
                child_depth, child_count = get_depth(child, depth+1)
                count += child_count
                max_depth = max_depth if max_depth > child_depth else child_depth
            return max_depth, count + 1

        def getBack(var_grad_fn):
            def depth_first(fn, prefix=""):
                print(prefix, fn)
                for child in [n[0] for n in fn.next_functions if n[0]]: depth_first(child, " "+prefix)
            print(depth_first(var_grad_fn, " "))
        # Since I'm dealing log probs rather than probs, must add together.
        x =  self.first_dist.log_prob(actions[:,0]) + self.second_dist.log_prob(actions[:,1]) 
        #print(getBack(x.grad_fn))
        #print(get_depth(x.grad_fn))
        #print("\n")
        return x
