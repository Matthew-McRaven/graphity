...
class SpinGlassSimulator(gym.Env):
    metadata = {}
    def __init__(self, H=graphity.environment.reward.IsingHamiltonian(), graph_size=4, allow_cuda=False, sampler=None):
        self.H = H
        self.graph_size = graph_size
        self.adj_mat = None
        self.allow_cuda = allow_cuda
    ...
    def reset(self, start_state=None):
        if start_state is not None:
            ...
            if self.sampler:
                ...
                # Otherwise depend on our utils facility to give us a good graph.
                self.state = graphity.graph.generate.random_graph(self.graph_size)
        # TODO: self.state = graphity.hypers.to_cuda(self.state, self.allow_cuda)
        # Simulation-internal state should not provide gradient feedback to system.
        self.state = self.state.requires_grad_(False)
        assert self.sampler
    ...
    def step(self, action):
        ...
        # For each index in the action list, apply the toggles.
        for (i,j) in action:
            graphity.environment.toggle.toggle_edge(int(i), int(j), next_state, allow_self_loop=True, enforce_symmetry=False)
